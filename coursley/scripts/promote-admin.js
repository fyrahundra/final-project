...
async function main() {
	...
	try {
		...
		if (existing.length > 0) {
			...
			console.log(
				`User exists (id=${id}). Updating account, promoting to admin, and setting instructor role.`
			);
			if (passwordHash) {
				await sql`
          	update "user"
		...
		console.log(
			'If you did not provide a password, a random password was generated (shown below). Save it securely:'
		);
		console.log(pwd);
		process.exit(0);
	} catch (err) {
